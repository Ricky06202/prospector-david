/** Dashboard de Prospector David (HTML + vanilla JS, acciones por data-* para evitar bugs de comillas). */
...
.hidden{display:none}
a.btn{display:inline-block;text-decoration:none;color:var(--b);font-weight:600}
</style>
</head>
<body>
<header>
...
function aviso(msg,tipo){
  const el=$('#aviso');
  el.textContent=msg;
  el.className='aviso'+(tipo==='err'?' av-err':'');
}
...
async function loadStats(t){
  ...
  ['nuevo','en_cola','enviado'].forEach(k=>{
    ...
    d.innerHTML='<b>'+t[k]+'</b>'+k.replace('_',' '); s.appendChild(d);
  });
  const d=document.createElement('div'); d.className='stat';
  d.innerHTML='<b>'+t.total+'</b>total'; s.appendChild(d);
}
...
async function cargarFotos(id){
  ...
  el.innerHTML=r.fotos.length
    ...
    : '<span class="muted">sin capturas</span>';
}
...
// Listener delegado: todas las acciones por data-accion (sin onclick inline).
